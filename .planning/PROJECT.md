...
---
*Last updated: 2026-05-03 after Phase 05 (Catalog — Meu Jardim) completion — 24 plans (23 original + post-UAT gap-closure plan 24), 16 requirement IDs delivered: 13 Complete (CAT-02,03,05,06,07,08,09,10,11 + OFF-08 + UI-04,07,11) + 3 Partial (CAT-01 schema branch awaits Phase 6 caller; CAT-04 + UI-08 cover/profile fields shipped, care-card/reminders/ID-history slots reserved for Phases 6/7/8). Shipped: pending_storage_deletions schema + Inngest catalog/cleanup-storage reconciler cron, catalog repositories (plants, photo-entries, location-suggestions), 11 route handlers under /api/v1/{plants,photos}, TanStack Query + IDB persister wired with i18n catalog namespace, useSubscription() stub for read-only mode harness, 5 new UI primitives (Combobox per WAI-ARIA APG 1.2, BottomSheet, Lightbox, InlineEditField, plant-grid with 2/3/4-col responsive breakpoints), Catalog page + Plant Profile page + Manual-add page + Photo-journal page + Home empty-state + /identify placeholder + Serwist runtime cache for offline browsing + read-only banner. 5 follow-up gap-closure plans (05-19..05-23) closed CR-01..CR-04 + WR-04..WR-05 surfaced during execution; plan 24 closed UAT gaps 3,4,5,6,8 (SSR mutations, combobox a11y, sort hook, SW order, journal i18n).*

*Last updated: 2026-04-29 after Phase 04 (IAM — Auth, Verification, Consent) completion — 13 plans (11 original + 04-12/04-13 gap closures), 19 requirement IDs delivered (AUTH-01..15, INFRA-10, NOTIF-01/02, UI-13). Schema additions: `email_verified_at` + `email_verification_tokens` + `password_reset_tokens` + `auth_throttle.locked_until`. Onboarded Inngest (9 functions: 8 PRD §3 MVP + 1 Phase-4 anti-enumeration password-reset job per D-11) and Resend (3 React Email templates + dev fallback). AuthAdapter is the sole `supabase.auth.*` boundary (Codex HIGH #3). Route-group gate at `(app)/layout.tsx` (Codex HIGH #7 replacing x-pathname). Six client auth pages via shared `useAuthForm` (D-31), Settings shell + Em breve placeholders, T&C/Privacy hyperlinks bound to active policy_version (Codex MEDIUM consent UX). Phase 02 (data-layer) and Phase 03 (design-system-app-shell) shipped earlier.*
