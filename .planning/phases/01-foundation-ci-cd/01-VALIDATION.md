...
| SC-4e | OBS-02 | 13 | 3 | PostHog US client + `posthog-node` server connected | unit + e2e | Unit: `tests/unit/posthog-config.test.ts` asserts factory uses `api_host: 'https://us.i.posthog.com'` + `opt_out_capturing_by_default: true` (D-17/D-18). E2E: smoke fires a server-side ping via `posthog-node` from a health endpoint, asserts HTTP 200 to `us.i.posthog.com` (Playwright `route()` mock OR PostHog events API) | ✅ | ⬜ pending |
...
- [ ] `tests/unit/posthog-config.test.ts` — asserts US host + `opt_out_capturing_by_default: true`
