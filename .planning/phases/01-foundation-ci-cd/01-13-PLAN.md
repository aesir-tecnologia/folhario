...
files_modified:
  ...
  - src/app/api/v1/_test/posthog-ping/route.ts
autonomous: true
...
must_haves:
  truths:
    ...
    - "`pnpm test:e2e` Playwright smoke covers D-24 full loop: HTML lang, health, test/throw route fires, security headers, server-side PostHog capture (R9 — posthog-node init + test_ping, NOT client-side posthog-js)"
...
<task type="auto">
  ...
  export const test = baseTest.extend<{ txDb: ReturnType<typeof drizzle> }>({
    ...
    txDb: async ({}, use) => {
      ...
      // R10 FRAGILITY WARNING — DO NOT "simplify" this spin loop away.
      //
      // This pattern works because porsager/postgres resolves the client.begin
      // callback synchronously once BEGIN lands on the wire. A future reader
      // might be tempted to replace the spin with a Promise-based deferred
      // resolution inside the begin callback — DON'T. That refactor looks
      // cleaner but (a) inverts the ownership of the fixture teardown path
      // (the begin callback must own the lifecycle for sql.begin's catch
      // handler to swallow the sentinel correctly), and (b) breaks in slow
      // CI environments where microtask ordering can delay resolution.
      //
      // The D-14 rollback contract IS this pattern. If it feels fragile, that
      // is because it is exercising a driver edge case. The unit tests in
      // this file are the backstop — if they pass, the pattern is correct.
      while (!capturedTxDb) {
...
<task type="auto">
  <name>Task 13.2: Create playwright.config.ts, smoke spec (with R9 server-side PostHog ping), and the posthog-ping test route</name>
  <files>playwright.config.ts, playwright/smoke.spec.ts, src/app/api/v1/_test/posthog-ping/route.ts</files>
  <read_first>
  ...
  <action>
  **R9 — first create the server-side PostHog ping route** the smoke will hit. This route is D-29-gated (same as `/api/v1/_test/throw`): it returns 404 unless `IDENTIFICATION_PROVIDER_MODE === 'stub'` AND `VERCEL_ENV === 'preview'`. On the happy path, it imports `posthog-node`, instantiates a server client (reusing whatever factory plan 08 exported), calls `capture({ event: 'test_ping', distinctId: 'ci-smoke' })`, and returns `{ ok: true }`.

  Create `src/app/api/v1/_test/posthog-ping/route.ts`:
  ...
  export async function GET() {
    ...
    try {
      client.capture({ event: 'test_ping', distinctId: 'ci-smoke' });
  ...
  test.describe('Phase 1 smoke — D-24 full observability loop', () => {
    ...
    test('PostHog server-side capture: posthog-node client initializes and emits test_ping from a server route (R9, SC-4e)', async ({ request }) => {
      // R9 — This assertion targets the SERVER-SIDE posthog-node client, NOT
      // a client-side posthog-js capture. The client-side SDK loads with
      // opt_out_capturing() by default per D-17 (LGPD Art. 33 consent gate),
      // so a browser-side capture would never fire without explicit opt-in.
      //
      // The smoke hits a dedicated test route that calls:
      //   posthogServer.capture({ event: 'test_ping', distinctId: 'ci-smoke' })
      // and returns 200. The route lives under /api/v1/_test/posthog-ping and
      // is gated on VERCEL_ENV === 'preview' AND IDENTIFICATION_PROVIDER_MODE === 'stub'
      // (same double-guard as /api/v1/_test/throw — D-29).
      //
      // This asserts two things at once:
      //   1. posthog-node (the server SDK) initializes without throwing
      //   2. capture() runs without a synchronous error
      // It does NOT assert that PostHog actually received the event — that
      // would require a PostHog API poll, which is too brittle for CI.
      // End-to-end reception is verified manually in plan 18.
      const res = await request.get('/api/v1/_test/posthog-ping');
      expect(res.status()).toBe(200);
      const body = await res.json();
  ...
  **SC-4a/SC-4b scope note (issue #5 fix):** Plan 13 is no longer listed as the
  ...
  <acceptance_criteria>
    ...
    - Test count >= 6 (home lang, service worker, security headers, health, test/throw, posthog-ping)
    - R9: `grep -q "posthog-ping" playwright/smoke.spec.ts` succeeds (server-side route, NOT client posthog-js)
    - R9: `grep -q "posthog-node" playwright/smoke.spec.ts` succeeds (test comment references the server SDK)
    - R10: `grep -q "R10 FRAGILITY WARNING" tests/integration/setup.ts` succeeds (warning comment on the spin loop)
