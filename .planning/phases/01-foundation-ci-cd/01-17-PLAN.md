...
must_haves:
  truths:
    ...
    - "R3 — BLOCKING post-deploy step `Verify Sentry release` queries Sentry releases API and FAILS the deploy when SENTRY_AUTH_TOKEN is non-empty AND the release has 0 artifacts (catches silent withSentryConfig upload failures). Skips gracefully when token is absent (local/first-boot runs)."
    - "R15 — Inngest sync uses the named CLI: `npx inngest-cli@latest deploy --url $APP_URL` (INNGEST_SIGNING_KEY required as a Wave 0 GitHub secret). Phase 1 asserts only that the endpoint returns valid JSON (R13 — functions: [] is Phase 1 scope; function-count assertion moves to Phase 2)."
...
**R3 — Sentry release self-check (BLOCKING):** `withSentryConfig` can silently fail to upload source maps (bad token, transient network error) without breaking the build. The BLOCKING post-deploy step queries the Sentry releases API for the `$GITHUB_SHA` release and FAILS the deploy job when SENTRY_AUTH_TOKEN is non-empty and the artifact count is zero. When the token is absent (local run, first-boot, dev) the step emits a `::notice::` and exits 0 to avoid false failures. This surfaces silent-upload failures at deploy time instead of during an incident.

**R15 — Inngest sync mechanism:** The sync step uses the named Inngest CLI command `npx inngest-cli@latest deploy --url $APP_URL`. This handles signature verification via the `INNGEST_SIGNING_KEY` env var, which is a REQUIRED Wave 0 GitHub Actions secret captured in plan 01. A raw `curl -X PUT $APP_URL/api/inngest` is kept as an independent sanity check that the endpoint returns valid JSON.

**R13 consequence on SC-3d:** Phase 1 ships `functions: []` in `src/app/api/inngest/route.ts` (plan 12 revision). The sync step therefore asserts only that the endpoint responds with valid JSON — NOT that function count >= 1. That assertion moves to Phase 2 when the first function registration lands.
...
<task type="auto">
  ...
  jobs:
    deploy:
      ...
      steps:
        ...
        - name: Verify Sentry release has source-map artifacts (R3, SC-3b, SC-3c)
          # R3 — BLOCKING when the token is non-empty AND the release has 0 artifacts.
          # If SENTRY_AUTH_TOKEN is absent (local test run, first-boot before Wave 0
          # secret is installed), we skip the assertion instead of false-failing.
          # When the token is present and the artifact count is zero, the deploy
          # job FAILS and surfaces the likely causes (bad token scopes, transient
          # upload error, misconfigured withSentryConfig).
          run: |
            if [ -z "$SENTRY_AUTH_TOKEN" ]; then
              echo "::notice::SENTRY_AUTH_TOKEN not set — skipping release verification (dev/local run)."
              exit 0
            fi
            RESPONSE=$(curl -sf \
              -H "Authorization: Bearer $SENTRY_AUTH_TOKEN" \
              "https://sentry.io/api/0/projects/$SENTRY_ORG/$SENTRY_PROJECT/releases/$GITHUB_SHA/files/" || echo "[]")
            ARTIFACT_COUNT=$(echo "$RESPONSE" | jq '. | length')
            echo "Sentry release $GITHUB_SHA has $ARTIFACT_COUNT artifact(s)."
            if [ -z "$ARTIFACT_COUNT" ] || [ "$ARTIFACT_COUNT" -lt 1 ]; then
              echo "::error::Sentry release $GITHUB_SHA has 0 artifacts. withSentryConfig source-map upload failed silently. Likely causes: (1) SENTRY_AUTH_TOKEN missing project:releases scope, (2) transient network failure, (3) misconfigured withSentryConfig options in next.config.ts. See plan 07 Task 7.3 (D-19 OVERRIDE verification) for throwaway-build reproduction."
              exit 1
            fi

        - name: Sync Inngest functions (R3, R15, SC-3d, R13 empty-array scope)
          # R15 — Inngest sync mechanism is explicit. We use the Inngest CLI
          # (`inngest-cli deploy`) rather than a raw curl PUT because the CLI
          # handles signature verification end-to-end via the INNGEST_SIGNING_KEY
          # environment variable. INNGEST_SIGNING_KEY is captured in the Wave 0
          # operator checklist (plan 01) and is a REQUIRED GitHub Actions secret.
          #
          # R13 — Phase 1 ships `functions: []`. The sync step therefore asserts
          # ONLY that the endpoint responds with valid JSON and a non-error HTTP
          # status. The function-count-ge-1 assertion moves to Phase 2 when the
          # first Inngest function registration lands. Phase 1 SC-3d is satisfied
          # by the sync fires successfully leg alone.
          env:
            INNGEST_SIGNING_KEY: ${{ secrets.INNGEST_SIGNING_KEY }}
            INNGEST_EVENT_KEY: ${{ secrets.INNGEST_EVENT_KEY }}
          run: |
            APP_URL="${{ steps.deploy.outputs.url }}/api/inngest"
            echo "Syncing Inngest functions at $APP_URL"
            # Named CLI: npx inngest-cli@latest deploy --url <app-url>
            # Alternative raw fallback: curl -sS -X PUT "$APP_URL" (kept for reference)
            npx --yes inngest-cli@latest deploy --url "$APP_URL"
            # Independent sanity check: the endpoint returns valid JSON.
            RESPONSE=$(curl -sS -X PUT -H "Content-Type: application/json" "$APP_URL")
            echo "Inngest endpoint response: $RESPONSE"
            # R13: Phase 1 function count may be 0 (functions: []). We only assert
            # the response parses as JSON — any non-JSON means the serve handler
            # is broken.
            echo "$RESPONSE" | jq . > /dev/null || { echo "::error::Inngest endpoint did not return valid JSON"; exit 1; }
  ```
  ...
  **Secret provenance check (must match Wave 0, plan 01):** Every `secrets.*` used by this workflow — `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, `SENTRY_PROJECT`, `NEXT_PUBLIC_SENTRY_DSN`, `NEXT_PUBLIC_POSTHOG_KEY`, `DATABASE_URL` — MUST appear in the Wave 0 operator checklist in plan 01. `SENTRY_ORG` and `SENTRY_PROJECT` are already present; `DATABASE_URL` is already present. If any future addition is needed, plan 01's checklist must be updated in the same revision. `PROD_DATABASE_URL` is explicitly FORBIDDEN — it does not exist.
  ...
  <acceptance_criteria>
    ...
    - R3: `grep -q "ARTIFACT_COUNT" .github/workflows/deploy-production.yml` succeeds (artifact count captured)
    - R3: `grep -q "SENTRY_AUTH_TOKEN" .github/workflows/deploy-production.yml` succeeds AND the verify step has an explicit empty-token skip branch (`grep -q "SENTRY_AUTH_TOKEN.*skipping" .github/workflows/deploy-production.yml` OR `grep -q "if \[ -z" .github/workflows/deploy-production.yml` succeeds)
    - R3: `grep -q "::error::Sentry release" .github/workflows/deploy-production.yml` succeeds (BLOCKING failure when token present + 0 artifacts)
    - R3: `grep -q "continue-on-error: true" .github/workflows/deploy-production.yml` in the Verify Sentry step is NO LONGER required (the step is BLOCKING) — it may still appear elsewhere in the workflow for unrelated steps
    - `grep -q "sentry.io/api/0/projects" .github/workflows/deploy-production.yml` succeeds (releases API URL)
    - R15: `grep -q "inngest-cli" .github/workflows/deploy-production.yml` succeeds (named CLI sync command)
    - R15: `grep -q "INNGEST_SIGNING_KEY" .github/workflows/deploy-production.yml` succeeds (required GH secret wired into the sync step env)
    - R13: `grep -q "FN_COUNT.*-lt 1" .github/workflows/deploy-production.yml` does NOT match (function-count >= 1 assertion removed — Phase 1 ships functions: [])
    - R13: the Inngest sync step asserts only that the endpoint returns valid JSON (grep: `grep -q "jq . > /dev/null" .github/workflows/deploy-production.yml` succeeds)
...
| T-17-3b | Tampering | Silent Sentry source-map upload failure | mitigate | R3 BLOCKING `Verify Sentry release` step queries Sentry releases API for artifact count; FAILS deploy when token is present and count is 0. Token-absent path emits `::notice::` and exits 0 (avoids false failures on local/first-boot runs). |
...
- R3: BLOCKING Sentry release artifact audit step — fails deploy when token is present and artifact count is 0; skips gracefully when token is absent
- R15: Inngest sync via named CLI (`npx inngest-cli@latest deploy`) with `INNGEST_SIGNING_KEY` required as a Wave 0 GH secret
- R13 consequence: Inngest sync asserts endpoint returns valid JSON only; function-count assertion moves to Phase 2 (Phase 1 ships `functions: []`)
