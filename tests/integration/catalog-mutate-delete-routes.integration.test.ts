...
describe.skipIf(!dbUrl || !supabaseAuthAvailable)(
  "Phase-05-09 catalog mutate/delete route handlers",
  () => {
    ...
    describe("DELETE /api/v1/plants/[plantId]", () => {
      ...
      it("Test 1 — happy path (cascade): 204 + cascade verified", async () => {
        ...
        // seed a reminder (cascades from plants — schema has no user_id column)
        await driver`
        INSERT INTO reminders (plant_id, type, frequency_days, next_due_at)
        VALUES (${plantId}, 'water', 7, ${new Date(Date.now() + 86400000).toISOString()})
      `;
      ...
      it("Test 4 — read-only mode bypass (T-05-09-04): 402 read_only_mode, use-case NOT called", async () => {
        ...
        const req = makeDeletePlantRequest({
          plantId,
          idempotencyKey: randomUUID(),
          readOnly: true,
        });
