...
- **No pre-existing code** at `src/`, no `package.json`, no `tsconfig.json`. Phase 1 creates the repo scaffold from zero. `.gitignore` contains only `.DS_Store` — Phase 1 must extend it (`node_modules`, `.env.local`, `.next`, `coverage`, `playwright-report`, `test-results`, `.supabase`, etc.). `next-env.d.ts` is gitignored and regenerated in CI via `pnpm exec next typegen` before `pnpm typecheck`, per Next 16 docs (`docs/01-app/03-api-reference/05-config/02-typescript.mdx` — *"We recommend adding `next-env.d.ts` to your `.gitignore` file"*). **SUPERSEDES Action 6 (2026-04-24):** Action 6 originally landed on "untracked AND not gitignored" to resolve a textual contradiction between 01-01-PLAN (gitignore) and 01-03-PLAN (track). That middle ground was never what the Codex review asked for (which was a clean binary: track OR gitignore) and conflicts with Next 16's own recommendation. Reverted post-merge — see STATE.md `Plan 01-08` decisions for full rationale.
