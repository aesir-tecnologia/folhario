...
export function IdentifyPlaceholder() {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-64px-80px)] flex-col items-center justify-center gap-4 px-5 py-8 text-center">
      <h1 className="font-serif text-[32px] leading-[38px] font-medium text-forest dark:text-moonpaper">
        {t("title")}
      </h1>
      {online ? (
        <p className="text-base text-slate dark:text-lantern-slate">{t("hint")}</p>
      ) : (
        <div
          role="status"
          aria-live="polite"
          className="flex items-center gap-2 bg-rust/10 px-4 py-2 text-sm text-rust rounded-md"
        >
