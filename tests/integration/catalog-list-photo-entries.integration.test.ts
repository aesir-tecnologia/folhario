...
describe.skipIf(!dbUrl || !supabaseAuthAvailable)(
  "Phase-05-07 listPhotoEntries use-case integration",
  () => {
    ...
    beforeAll(async () => {
      ({ listPhotoEntries } = await import("@contexts/catalog/application/list-photo-entries"));
    ...
    it("Test 5 (CR-03): result items expose photoSignedUrl alongside thumbnailSignedUrl", async () => {
      ...
      expect(typeof item.thumbnailSignedUrl === "string" || item.thumbnailSignedUrl === null).toBe(
        true,
      );
