...
| **Full suite command** | `pnpm lint && pnpm typecheck && pnpm test:unit -- --run && pnpm test:integration -- --run && pnpm build` |
...
> **Watch-mode ban:** `--run` flag is MANDATORY for every vitest invocation (CLAUDE.md rule). No `pnpm test` or `pnpm vitest` without `--run` anywhere in scripts, hooks, or CI.

---

## Sampling Rate

- **After every task commit:** Run `pnpm test:unit -- --run` (quick path, <30s)
...
| SC-2b | INFRA-12 | TBD | TBD | Integration vs `postgres:17-alpine` service container with drizzle-kit migrate | integration | `pnpm test:integration -- --run` — at minimum `tests/integration/health.test.ts` executes `SELECT 1` through the Drizzle client | ✅ | ⬜ pending |
| SC-2c | INFRA-13 | TBD | TBD | Playwright vs preview URL bound to Supabase branch DB | e2e | `deploy-preview.yml` Playwright job green; smoke asserts `GET /api/v1/health` → `{ status: 'ok', db: 'ok' }` on preview URL | ✅ | ⬜ pending |
...
| SC-3a | INFRA-14 | TBD | TBD | `main` merge deploys via `vercel deploy --prebuilt --prod` | CI | `deploy-production.yml` green on a staged merge | ✅ | ⬜ pending |
| SC-3b | OBS-01 | TBD | TBD | Sentry release tagged with git SHA | assertion | `withSentryConfig` native upload fails build on error; post-deploy `curl https://sentry.io/api/0/projects/{org}/{proj}/releases/{sha}/` returns 200 | ✅ | ⬜ pending |
| SC-3c | OBS-01 | TBD | TBD | Turbopack source maps uploaded post-build via `withSentryConfig` (D-19 OVERRIDE) | assertion | Sentry release has >0 artifacts (`GET /releases/{sha}/files/`) | ✅ | ⬜ pending |
| SC-3d | INFRA-10 (cross-phase) / INFRA-14 | TBD | TBD | Inngest registration endpoint reachable (hello-world function) | assertion | `deploy-production.yml` step calls `curl -X PUT https://{prod}/api/inngest` — returns 200 with function count ≥ 1 | ✅ | ⬜ pending |
| SC-3e | INFRA-11 | TBD | TBD | Vercel git integration confirmed OFF | manual | Wave 0 operator checklist screenshot of Vercel project settings; re-verified in Phase 1 closing note | — | ⚠ manual |
| SC-4a | LGPD-13, D-24 | TBD | TBD | Deliberately thrown error reaches Sentry | e2e | Playwright hits `GET /api/v1/_test/throw`, waits for Sentry ingest, queries Sentry issues API for an issue tagged with the test run's `trace_id`, asserts it exists within 10s | ✅ | ⬜ pending |
| SC-4b | LGPD-13 | TBD | TBD | Scrubbed fields absent from Sentry payload | e2e | Same Playwright test asserts event body contains `[Filtered]` where `email` / `password` / `photo_url` / `token` / `Authorization` / `Cookie` would appear; `trace_id` IS present (control) | ✅ | ⬜ pending |
...
- [ ] `tests/unit/sentry-config.test.ts` — asserts `beforeSend` scrubbing (LGPD-13) + `setUser({ id })` only
...
- [ ] `tests/integration/setup.ts` — per-file `drizzle-kit migrate` + per-test transaction wrapper rolled back on teardown (D-14)
- [ ] `tests/integration/health.test.ts` — real Postgres via `postgres-js` with `{ prepare: false }` → `SELECT 1`
- [ ] `playwright/smoke.spec.ts` — D-24 full observability loop (home, `/api/v1/health`, `/api/v1/_test/throw`, PostHog ping, security headers, LGPD-13 scrubbing assertion)
...
- [ ] `.github/workflows/deploy-production.yml` — `vercel deploy --prebuilt --prod` + Inngest sync (Sentry release + source-map upload handled by `withSentryConfig` in `next build`, not a separate step — per D-19 OVERRIDE)
...
| Sentry project receives LGPD-13 scrubbed event from preview | LGPD-13 | Final-mile verification that scrubbing works end-to-end on real Sentry infrastructure (not mocked) | After Phase 1 preview deploy, visit Sentry → Issues → find the deliberate error → confirm `request.data === undefined` and scrubbed fields show `[Filtered]`. |
