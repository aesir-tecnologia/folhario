...
- **`tests/e2e/auth-password-reset.spec.ts`** (Codex HIGH #6): seed verified user → reset-request → mint via diagnostics → submit reset → login with new password (200) → login with old password (401 invalid_credentials). Second test: non-existent email → 200 (D-11).
  ...
  **Scope decision — E2E shortcuts the Inngest pipeline:** the spec mints the reset token via the `latest-reset-token` diagnostics endpoint (which calls the same `mintResetToken` repo a real Inngest run would invoke) rather than waiting for a running `pnpm inngest:dev` instance to dispatch the `iam/password-reset-requested` event end-to-end. Coverage trade-off:
  - **What the E2E exercises**: cookie-bearing reset-request → consume route → adapter → DB tx → login-with-new-password vs login-with-old-password.
  - **What the E2E does NOT exercise end-to-end**: the `iam/password-reset-requested` Inngest function → `requestPasswordReset` use-case → `notifications/email.requested` chain. That pathway IS exercised by `tests/integration/iam-password-reset.integration.test.ts` (via direct `requestPasswordReset` invocation) — so the coverage exists, but in two complementary tests rather than one. This decouples CI from a running Inngest dev runtime while keeping the cookie-bearing flow's correctness witnessed.
