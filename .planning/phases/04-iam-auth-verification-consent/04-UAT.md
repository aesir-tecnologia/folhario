...
result: pass
verified_via: "Fixed in commits 426653f (force INNGEST_DEV=1 in pnpm dev script) + 55fb177 (decouple inngest.send from signup write-path + Sentry.captureException replaces bare catch). Regression coverage: tests/e2e/auth-signup-verification.spec.ts; tests/integration/iam-signup.integration.test.ts:90 (organic happy path, 14-day trial, 5 row groups + verification Inngest event)."
prior_result: "issue — POST /api/v1/iam/signup 500 in 3.3s; bare catch swallowed Inngest cloud-mode delivery failure against placeholder INNGEST_EVENT_KEY"
...
result: pass
verified_via: "Resolved by symmetry with test 2 (commits 426653f + 55fb177). Regression coverage: tests/e2e/auth-signup-active-partner-code.spec.ts; tests/integration/iam-signup.integration.test.ts:189 (D-32 active partner_code → trial_source='partner' + ~30-day trial)."
prior_result: "issue — same Inngest cloud-mode 500 as test 2"
related_to: 2
...
result: pass
verified_via: "tests/e2e/auth-signup-invalid-partner-code.spec.ts"
...
result: pass
verified_via: "tests/e2e/auth-signup-verification.spec.ts (already_registered branch); tests/integration/iam-signup.integration.test.ts:164 (use-case + welcome-back dispatch)"
...
result: pass
verified_via: "tests/integration/notifications-send-email.integration.test.ts; tests/integration/iam-resend-verification.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/iam-unverified-blocker.spec.ts; tests/integration/iam-verification-gate.integration.test.ts"
...
result: pass
verified_via: "tests/integration/iam-resend-verification.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/auth-signup-verification.spec.ts; tests/integration/iam-verify-token.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/auth-login-logout.spec.ts; tests/integration/iam-login.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/auth-login-invalid-credentials.spec.ts"
...
result: pass
verified_via: "tests/e2e/auth-login-rate-limit-trip.spec.ts; tests/integration/iam-throttle.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/auth-login-logout.spec.ts"
...
result: pass
verified_via: "tests/e2e/auth-password-reset.spec.ts; tests/integration/iam-password-reset-route-timing.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/auth-password-reset.spec.ts; tests/integration/iam-password-reset.integration.test.ts; tests/integration/iam-password-reset-tx-rollback.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/auth-change-password.spec.ts; tests/integration/iam-change-password.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/auth-change-password-oauth-only-forbidden.spec.ts"
...
result: pass
verified_via: "tests/e2e/auth-google-oauth.spec.ts; tests/integration/iam-oauth.integration.test.ts"
...
result: pass
verified_via: "tests/e2e/iam-me-shape.spec.ts"
...
result: pass
verified_via: "tests/e2e/iam-me-update-timezone.spec.ts"
...
result: pass
verified_via: "tests/e2e/settings-account.spec.ts:40-42"
...
result: pass
verified_via: "tests/e2e/settings-account.spec.ts:91; tests/e2e/axe-placeholder-pages.spec.ts"
...
result: pass
verified_via: "tests/e2e/legal-links.spec.ts"
...
result: pass
verified_via: "tests/e2e/auth-signup-blocks-without-consent.spec.ts"
...
result: pass
verified_via: "tests/e2e/auth-public-redirect-when-verified.spec.ts"
...
- truth: "POST /api/v1/iam/signup with valid organic-signup body returns 200 and lands the user on UnverifiedBlocker"
  ...
  resolved_at: 2026-04-29
  resolution: "Quickwin (426653f): pnpm dev script now sets INNGEST_DEV=1 so the SDK no-ops to the local dev server instead of attempting cloud delivery against the placeholder INNGEST_EVENT_KEY. Hardening (55fb177): inngest.send decoupled from signup write-path; bare catch at route.ts:99-101 replaced with Sentry.captureException so any future delivery fault is observable rather than silently 500ing a signup whose DB writes already committed. Regression coverage: tests/e2e/auth-signup-verification.spec.ts + tests/integration/iam-signup.integration.test.ts:90."
  fix_commits:
    - 426653f
    - 55fb177
  severity: blocker
...
- truth: "POST /api/v1/iam/signup with valid active partner_code body returns 200 and applies 30-day trial"
  ...
  resolved_at: 2026-04-29
  resolution: "Resolved by symmetry with test 2 fix (commits 426653f + 55fb177). Regression coverage: tests/e2e/auth-signup-active-partner-code.spec.ts (~30d trial_end_date assertion) + tests/integration/iam-signup.integration.test.ts:189 (D-32 active partner_code → trial_source='partner')."
  fix_commits:
    - 426653f
    - 55fb177
  severity: blocker
