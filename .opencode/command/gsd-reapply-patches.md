...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gsd-local-patches" ]; then
    ...
    PATCHES_DIR="/Users/machado/Projects/folhario/.opencode/gsd-local-patches"
  fi
fi
...
- **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `/Users/machado/Projects/folhario/.opencode/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
