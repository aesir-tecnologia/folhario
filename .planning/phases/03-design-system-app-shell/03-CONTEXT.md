...
- **D-32:** Banned-pattern enforcement is distributed and wired into commands: **Stylelint with custom rules** (color/typography/animation patterns) **+ ESLint custom rule** (emoji + banned imports like Inter / Times / Georgia / generic serifs) **+ Vitest snapshot rule** (cross-cutting font + token presence checks). Phase 3 adds `pnpm lint:styles`, includes it in the CI verification path, and extends `lint-staged` so CSS files run Stylelint while TS/TSX files run ESLint. CI runs the full suite. **[Plan 03-01 substitution — Open Risk #1]:** The original D-32 text referenced `eslint-plugin-tailwindcss` as the Tailwind lint plugin, but that package has no stable Tailwind v4-ready release. Plan 03-01 ships `eslint-plugin-better-tailwindcss@4.4.1` instead. All D-32 enforcement goals are met by the substituted plugin.
