...
| ------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/unit/use-online-status.test.tsx`            | UI-24      | Heartbeat hits `/api/v1/health/connectivity` literally; offline after 1 failed ping; backoff to 60 s after success; never `/api/v1/diagnostics/ping` (CI-stub gated). |
...
| `tests/unit/app-update-toast.test.tsx`            | OFF-10     | (B-3 fix; replaces e2e smoke) Verifies the OFF-10 contract end-to-end in jsdom: subscription order (`waiting` listener attached BEFORE `register()`), sonner toast renders with `app.update.label` + `app.update.cta`, click calls `messageSkipWaiting()`, `controlling` event triggers injected `reload()`. |
...
| `tests/e2e/sw-update-toast.spec.ts` (RETIRED — see `tests/unit/app-update-toast.test.tsx`) | OFF-10 | (B-3 fix) The OFF-10 contract is now owned by the unit-dom test in Plan 04. Synthesizing real SW `waiting` events in headless Chromium requires shipping a second SW build per test run — too costly for the marginal fidelity gain over the dependency-injected jsdom test. End-to-end SW lifecycle smoke is owned by manual UAT (Lighthouse PWA + iOS Safari install). |
...
| `tests/e2e/axe-modal-focus-trap.spec.ts`  | UI-22                | (B-4 fix) Navigate to Plan 03 test route `/modal-sheet` (`(test)/modal-sheet/page.tsx`), wait for `[role="dialog"]`, assert initial focus is inside the Dialog, Tab N times asserting focus stays inside Dialog, press Escape to close, assert focus returns to `data-testid="modal-invoker"`.                                                                                                                    |
