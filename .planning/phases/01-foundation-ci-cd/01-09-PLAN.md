...
must_haves:
  truths:
    ...
    - "CSP connect-src allowlist includes sentry.io, us.i.posthog.com, *.supabase.co, api.stripe.com"
...
**R14 — HSTS preload rationale (plan text, NOT a code comment):** The HSTS value is `max-age=15552000; includeSubDomains` — deliberately WITHOUT `; preload`. HSTS preload list inclusion via hstspreload.org is a browser-level commitment that is operationally irreversible (removal requests take 6+ months to propagate across major browsers) and is therefore inappropriate for a new domain that may still need to change subdomains, DNS providers, or TLS setup during early operation. A future reader who wants to "complete the header" by adding `preload` MUST first confirm the domain and all subdomains will serve valid HTTPS forever, then go through the hstspreload.org submission. Until then, `includeSubDomains` without `preload` gives 95% of the protection with none of the lock-in. This note lives in plan text (not `src/shared/http/headers.ts` comments) per project convention — code comments are reserved for complex logic, not policy rationale.

**Plan-03 note location correction (R14 revision traceability):** The revision table item R14 was originally scoped to "Plan 01-03 (tooling)". R14's intent — explaining the HSTS max-age + no-preload choice — only makes sense where the HSTS header is actually set, which is plan 01-09. The note has been recorded here in plan 09's reasoning section. Plan 03 is unchanged.
...
<feature>
  ...
  `src/shared/http/headers.ts` exports:
    - `CSP_DIRECTIVES` — Record<string, string[]> with:
      ...
      - `font-src`: `["'self'"]`
      - `connect-src`: `["'self'", 'https://*.sentry.io', 'https://*.ingest.sentry.io', 'https://us.i.posthog.com', 'https://us-assets.i.posthog.com', 'https://*.supabase.co', 'wss://*.supabase.co', 'https://api.stripe.com']`
  ...
  Test cases (on `buildSecurityHeaders`):
    ...
    - CSP string contains `https://us.i.posthog.com`
...
<task type="auto" tdd="true">
  ...
  describe('buildSecurityHeaders (INFRA-18, D-20..D-22)', () => {
    ...
    it('CSP connect-src allowlist includes sentry, posthog US, supabase, stripe', () => {
      const h = buildSecurityHeaders('production');
