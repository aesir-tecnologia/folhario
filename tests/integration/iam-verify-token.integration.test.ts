...
describe.skipIf(!dbUrl || !supabaseAuthAvailable)(
  "Phase 4 plan 06 — verifyEmail (AUTH-04 + Codex HIGH #2 ordering)",
  () => {
    beforeAll(async () => {
    ...
    async function mintTokenForUser(userId: string, email: string): Promise<string> {
      const { mintVerificationToken } =
        await import("@contexts/iam/infrastructure/db/verification-tokens");
    ...
    it("tx rollback: if setEmailVerifiedAt throws, consumed_at is NOT set", async () => {
      ...
      const spy = vi.spyOn(usersMod, "setEmailVerifiedAt").mockImplementationOnce(async () => {
