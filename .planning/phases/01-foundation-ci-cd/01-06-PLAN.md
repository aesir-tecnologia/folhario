...
<feature>
  ...
  **Public (NEXT_PUBLIC_*):**
    ...
    - `NEXT_PUBLIC_POSTHOG_HOST` — `z.string().url().default('https://us.i.posthog.com')` AND must contain `us.i.posthog.com` (OBS-02: US cloud chosen for lower latency from Brazil; LGPD Art. 33 transfer basis via PostHog SCCs)
  ...
  **Vercel runtime (auto-set):**
  ...
  Test cases:
    ...
    - `NEXT_PUBLIC_POSTHOG_HOST='https://eu.i.posthog.com'` → throws (must contain `us.i.posthog.com` for OBS-02)
...
<task type="auto" tdd="true">
  ...
  describe('env parser (INFRA-16, INFRA-17, D-24, D-29, OBS-02)', () => {
    ...
    it('rejects PostHog EU host (OBS-02 US cloud locked for latency)', async () => {
      setEnv({ NEXT_PUBLIC_POSTHOG_HOST: 'https://eu.i.posthog.com' });
      await expect(loadEnv()).rejects.toThrow(/us\.i\.posthog\.com/);
    });
  ...
  // R18 — explicit parity matrix for the two operational shapes of the two DB URL vars.
  describe('local parity (DATABASE_POOL_URL === DATABASE_URL — local Docker case, no Supavisor)', () => {
    it('parses without error when both URLs resolve to the same direct postgres URL', async () => {
      const LOCAL = 'postgresql://postgres:postgres@localhost:5432/folhario_test';
      setEnv({ DATABASE_URL: LOCAL, DATABASE_POOL_URL: LOCAL });
      const env = await loadEnv();
      expect(env.DATABASE_URL).toBe(LOCAL);
      expect(env.DATABASE_POOL_URL).toBe(LOCAL);
    });
  });

  describe('production split (DATABASE_POOL_URL !== DATABASE_URL — Supavisor pooler + direct bypass)', () => {
    it('parses without error when pool URL is the Supavisor pooler and direct URL is the bypass connection', async () => {
      const DIRECT = 'postgresql://postgres:pw@db.abc.supabase.co:5432/postgres';
      const POOL = 'postgresql://postgres.abc:pw@aws-0-sa-east-1.pooler.supabase.com:6543/postgres';
      setEnv({ DATABASE_URL: DIRECT, DATABASE_POOL_URL: POOL });
      const env = await loadEnv();
      expect(env.DATABASE_URL).toBe(DIRECT);
      expect(env.DATABASE_POOL_URL).toBe(POOL);
      expect(env.DATABASE_URL).not.toBe(env.DATABASE_POOL_URL);
    });
  });
  ```
  ...
  <acceptance_criteria>
    ...
    - 10+ `it(...)` blocks (original 8 + 2 new parity cases)
    - R18: `grep -q "describe('local parity" tests/unit/env-provider-mode.test.ts` succeeds
    - R18: `grep -q "describe('production split" tests/unit/env-provider-mode.test.ts` succeeds
  </acceptance_criteria>
...
<task type="auto" tdd="true">
  ...
  const schema = z.object({
    ...
    NEXT_PUBLIC_POSTHOG_HOST: z
      ...
      .refine((v) => v.includes('us.i.posthog.com'), {
        message: 'NEXT_PUBLIC_POSTHOG_HOST must contain us.i.posthog.com (OBS-02: US cloud locked for lower latency from Brazil; LGPD Art. 33 transfer basis via PostHog SCCs)',
      }),
...
| T-06-LGPD | Privacy | PostHog host pinned | mitigate | Zod refinement requires `us.i.posthog.com` substring (OBS-02: US cloud locked for latency; LGPD Art. 33 transfer basis via PostHog SCCs); unit test covers |
