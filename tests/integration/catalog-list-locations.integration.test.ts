...
describe.skipIf(!dbUrl || !supabaseAuthAvailable)(
  "Phase-05-07 listLocations use-case integration",
  () => {
    ...
    it("Test 5: de-dupe case-insensitive — 'SALA' + default 'Sala' → one entry", async () => {
      ...
      const salaCount = labels.filter(
        (l) => l.normalize("NFKC").toLowerCase().trim() === "sala",
      ).length;
    ...
    it("Test 6: unicode normalization — 'Escritório' in any normalization → one entry", async () => {
      ...
      expect(result.ok).toBe(true);
      const labels = result.items.map((i) => i.labelDisplay);
      const escritorioCount = labels.filter(
        (l) => l.normalize("NFKC").toLowerCase().trim() === "escritório",
      ).length;
