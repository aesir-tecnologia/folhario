...
**Success Criteria** (what must be TRUE):
  ...
  4. A deliberately thrown error in any environment appears in Sentry with `Authorization`, `Cookie`, `email`, `password`, `token`, and `photo_url` scrubbed, `Sentry.setUser({ id })` only, and request bodies dropped on `/api/v1/identifications/*` routes; PostHog US client + `posthog-node` server are connected and a ping event lands in the US project.
...
**Plans**: 18 plans
- [ ] 01-01-PLAN.md — Wave 0 operator provisioning checklist (Sentry/PostHog US/Supabase/Vercel/Inngest + GH Actions secrets + Node 22/pnpm) — D-27 [autonomous: false]
