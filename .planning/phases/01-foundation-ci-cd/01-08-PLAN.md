...
<objective>
TDD plan: build client (`posthog-js`) and server (`posthog-node`) PostHog factories that enforce D-17 (consent-gated opt-out default), D-18 (autocapture off), and OBS-02 (US cloud pinned for lower latency from Brazil; LGPD Art. 33 transfer basis via PostHog SCCs). Both factories expose testable config constants so unit tests can assert values WITHOUT initializing the SDK (which would require a DOM / fetch call).
...
export const env: Readonly<{
  ...
  NEXT_PUBLIC_POSTHOG_HOST: string; // defaults to https://us.i.posthog.com, validated to contain us.i.posthog.com
}>;
```
</interfaces>
</context>
...
<feature>
  ...
  `posthog-server.ts`:
    - Exports `getPostHogServer(): PostHog` — singleton over `posthog-node`'s `PostHog` class, `host: 'https://us.i.posthog.com'`; lazy init
    - Subsequent calls return the same instance

  Test cases:
    - `POSTHOG_CLIENT_CONFIG.api_host === 'https://us.i.posthog.com'`
  ...
  <implementation>
  Client factory returns null in non-browser environments to avoid `window` errors. Server factory uses `posthog-node` which works in Node without DOM. Both honor OBS-02 US host.
  </implementation>
</feature>
...
<task type="auto" tdd="true">
  ...
  describe('POSTHOG_CLIENT_CONFIG (OBS-02, D-17, D-18)', () => {
    it('uses US host (OBS-02 US cloud; LGPD Art. 33 transfer basis via SCCs)', async () => {
      const { POSTHOG_CLIENT_CONFIG } = await import('@/shared/telemetry/posthog-client');
      expect(POSTHOG_CLIENT_CONFIG.api_host).toBe('https://us.i.posthog.com');
    });
...
<task type="auto" tdd="true">
  ...
  <acceptance_criteria>
    ...
    - `grep -q "api_host: 'https://us.i.posthog.com'" src/shared/telemetry/posthog-client.ts` succeeds
    ...
    - `grep -q "host: 'https://us.i.posthog.com'" src/shared/telemetry/posthog-server.ts` succeeds
...
| T-08-Residency | Privacy (LGPD) | Event data geography | mitigate | Hardcoded `us.i.posthog.com` in both factories + env parser refinement in plan 06 |
</threat_model>
