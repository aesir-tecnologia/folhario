...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <read_first>
    - .planning/phases/01-foundation-ci-cd/01-18-VERIFICATION.md (checklist initialized by Task 18.1)
    - .planning/phases/01-foundation-ci-cd/01-17-BRANCH-PROTECTION.md (gh api commands for item 4)
    - .planning/phases/01-foundation-ci-cd/01-VALIDATION.md (SC-2d, SC-3e, SC-4a, SC-4b rubrics)
  </read_first>
  <action>
  Step 1 — Executor opens `.planning/phases/01-foundation-ci-cd/01-18-VERIFICATION.md` (initialized by Task 18.1) and appends the 5-item checklist below verbatim under a `## Operator Checklist` heading, each item with empty `Screenshot:` and `Date:` fields the operator fills in.

  Step 2 — Executor prints the full `<how-to-verify>` block (below) to the operator and halts with the checkpoint message:
  ```
  CHECKPOINT: Phase 1 Operator Verification (Blocking)

  Throwaway PR is green. Complete the 5 manual verifications in
  .planning/phases/01-foundation-ci-cd/01-18-VERIFICATION.md, then reply:
    - "verified" if all 5 items are recorded with screenshots + dates, OR
    - "failed: {reason}" if any check failed (enters gap-closure planning)
  ```

  Step 3 — Executor does NOT poll. It waits for the explicit resume signal defined in `<resume-signal>` below. The executor MUST NOT mark this task complete on any reply other than `verified`.

  Step 4 — On `verified`, executor reads VERIFICATION.md, confirms all 5 `Screenshot:` fields are non-empty and all 5 `Date:` fields are ISO-8601, then writes `01-18-SUMMARY.md` per the `<output>` block.

  Step 5 — On `failed: {reason}`, executor records the failure in VERIFICATION.md and hands off to gap-closure (`/gsd-plan-phase 1 --gaps`).
  </action>
  <how-to-verify>
  ...
  - Inngest dashboard — confirm the `folhario` app is registered and shows 0 synced functions (Phase 1 ships bare `serve({ client: inngest, functions: [] })` per R13; function registration lands in Phase 2) (SC-3d)
  - Screenshot both.
  ...
  <acceptance_criteria>
    - `.planning/phases/01-foundation-ci-cd/01-18-VERIFICATION.md` contains heading `## Operator Checklist`
    - VERIFICATION.md contains the literal strings `SC-3e`, `SC-2d`, `SC-4a`, `SC-4b`, `D-11` (all 5 check IDs present)
    - VERIFICATION.md contains at least 5 non-empty `Screenshot:` lines (grep-verifiable: `grep -c '^Screenshot: .\+$' VERIFICATION.md` >= 5)
    - VERIFICATION.md contains at least 5 `Date: 20` lines (ISO-8601 year prefix, grep-verifiable: `grep -c '^Date: 20' VERIFICATION.md` >= 5)
    - VERIFICATION.md contains the string `functions: []` or `0 synced functions` in the Inngest item (R13 regression guard — NOT `1 synced function` or `hello-world`)
    - Operator reply is exactly `verified` (resume-signal contract)
    - `01-18-SUMMARY.md` exists after the task completes
  </acceptance_criteria>
  <resume-signal>
