...
describe.skipIf(!dbUrl || !supabaseAuthAvailable)(
  "Phase 4 plan 09 — OAuth callback + complete (AUTH-03 + D-04 + D-32 + Codex HIGH #2/#3)",
  () => {
    beforeAll(async () => {
    ...
    describe("GET /auth/callback (Codex HIGH #3 + Resolved Q4)", () => {
      it("error= param → redirects /auth/login?error=oauth_failed", async () => {
        ...
        expect(res.headers.get("location")).toContain("/auth/login?error=oauth_failed");
    ...
    describe("POST /api/v1/iam/oauth/complete (D-04 + D-32 + Codex HIGH #2)", () => {
      ...
      it("idempotent: second call after success returns 200 (already_completed)", async () => {
        ...
        const subRows = await cleanupSql!<
          { id: string }[]
        >`SELECT id FROM public.subscriptions WHERE user_id = ${id}`;
        expect(subRows).toHaveLength(1);
      });
      ...
      it("Codex HIGH #2 tx rollback: insertSignupConsents throws → age_confirmed_at NOT set", async () => {
        ...
        const consentMod = await import("@contexts/iam/infrastructure/db/consent-logs");
        const spy = vi
        ...
        expect(userRows[0]!.email_verified_at).toBeNull();
        const consentRows =
          await cleanupSql!`SELECT id FROM public.consent_logs WHERE user_id = ${id}`;
        expect(consentRows).toHaveLength(0);
        const subRows =
          await cleanupSql!`SELECT id FROM public.subscriptions WHERE user_id = ${id}`;
